...
# Create file handler for lint_engine module
fh = logging.FileHandler('lint_engine_forensics.log')
...
def getDataLoadCount( py_file ):
    logger.info(f"FORENSICS: Starting data load count analysis for file: {py_file}")
    data_load_count = 0 
    ...
    logger.info(f"FORENSICS: Data load count analysis complete for {py_file} - Found {data_load_count} data load operations, Logging enabled: {LOGGING_IS_ON_FLAG}")
