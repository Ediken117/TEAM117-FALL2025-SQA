...
# Create file handler for py_parser module
fh = logging.FileHandler('py_parser_forensics.log')
...
def getPythonParseObject( pyFile ):
	logger.info(f"FORENSICS: Parsing Python file: {pyFile}")
	try:
		full_tree = ast.parse( open( pyFile ).read())
		logger.info(f"FORENSICS: Successfully parsed {pyFile}")
	except SyntaxError as e:
		# print(constants.PARSING_ERROR_KW, pyFile )
		logger.warning(f"FORENSICS: Syntax error in {pyFile}: {str(e)}")
