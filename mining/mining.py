...
# Configure logging for forensics
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

# Create file handler for mining module
fh = logging.FileHandler('mining_forensics.log')
...
def dumpContentIntoFile(strP, fileP):
    logger.info(f"FORENSICS: Writing content to file: {fileP}")
    fileToWrite = open( fileP, 'w')
    fileToWrite.write(strP )
    fileToWrite.close()
    file_size = str(os.stat(fileP).st_size)
    logger.info(f"FORENSICS: Successfully wrote {file_size} bytes to {fileP}")
    return file_size
...
def days_between(d1_, d2_): ## pass in date time objects, if string see commented code 
    ...
    logger.info(f"FORENSICS: Calculating days between dates - Start: {d1_}, End: {d2_}")
    days = abs((d2_ - d1_).days)
    logger.info(f"FORENSICS: Days between calculation result: {days} days")
    return days
...
def getPythonFileCount(path2dir):
    logger.info(f"FORENSICS: Counting Python files in directory: {path2dir}")
    valid_list = [] 
    ...
    logger.info(f"FORENSICS: Found {count} Python files (.py and .ipynb) in {path2dir}")
    return count   
